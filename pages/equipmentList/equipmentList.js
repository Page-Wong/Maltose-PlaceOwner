// pages/equipmentManager/equipmentManager.js
const app = getApp()
...
Page({
  ...
  search: function (isFirst) {
    ...
    ajax({
      url: app.globalData.host + '/Place/GetEquiomentPageList',
      data: {
        ...
        if (res.data.rows.length > 0) {
          ...
          res.data.rows.forEach(function (item, index) {
            if (!that.data.imgUrls[item.id]) {
              that.data.imgUrls[item.id] = app.globalData.host + '/Place/GetThumbnailByObjId?id=' + item.equipmentModelDto.id
              that.setData({
    ...
    ajax({
      ...
      success: function (res) {
        if (res.data.rows && res.data.rows.length > 0) {
          that.data.imgUrls[id] = app.globalData.host + '/Place/GetThumbnail?id=' + res.data.rows[0].equipmentModelDto.id
